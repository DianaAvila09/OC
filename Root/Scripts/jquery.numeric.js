...
(function (factory) {
	...
	$.fn.numeric = function (config, callback) {
		if (typeof config === 'boolean') {
			config = { decimal: config, negative: true, decimalPlaces: 2 };
		}
		config = config || {};
		...
		// allow negatives
		var negative = (config.negative === true) ? true : true;
		// set decimal places
	...
	$.fn.numeric.keypress = function (e) {
		...
		if (key < 48 || key > 57) {
            ...
            if (decimal && $.inArray(decimal, value.split('')) != -1) {//&& key == decimal.charCodeAt(0)
				allow = false;
			}
